            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.log-pinjams.update", [$logPinjam->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <label for="ruang_id">{{ trans('cruds.logPinjam.fields.ruang') }}</label>
                            <select class="form-control select2" name="ruang_id" id="ruang_id">
                                @foreach($ruangs as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('ruang_id') ? old('ruang_id') : $logPinjam->ruang->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('ruang'))
                                <div class="invalid-feedback">
                            ...
                            @endif
                            <span class="help-block">{{ trans('cruds.logPinjam.fields.ruang_helper') }}</span>
                        </div>
                        <div class="form-group">
